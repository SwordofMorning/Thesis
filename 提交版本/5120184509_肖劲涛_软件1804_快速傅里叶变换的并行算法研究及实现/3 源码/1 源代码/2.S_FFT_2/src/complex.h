/* 
    本头文件提供复数类complex_t的定义
    
    ====================

    This head file provides complex_t
*/
...
/*
    复数类，相比于std::complex提供了*模板化*数据类型

    =====

    compex class, provides template
*/
template<typename T>
class complex_t {
private:
...
/*
    重载左移运算符，添加complex_t类型

    =====

    Overload << for complex_t
*/
template<typename T>
...
namespace std {
    ...
    T arg(const complex_t<T>& c) {
        // std::atan2(y, x), (-pi ~ pi)，返回(x, y)的反正切值
