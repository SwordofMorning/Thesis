#pragma once
/*
*   FFT 单线程 1 | FFT Signal Threads
*   此文件使用complex_t完成fft算法，算法表如下：
*	1. naive fft/ifft
...
/*
   L2型蝶
   参数：a, b两个输入量；w，旋转因子
   Params: a,b input Val; w, twiddle factor
*/
#define BUFL2(a,b,w)                \
    do{                             \
        complex_t<T> temp = (a);    \
...
/*
    ...
    参数：时域序列、频域序列、序列长度
    Params: Time seq, Frequency seq, seqLenth
*/
template<typename T>
void fft_naive(const std::vector<complex_t<T>>& t_seq, std::vector<complex_t<T>>& f_seq, size_t length = 0) 
{
    /*
        ...
        Naive FFT twiddle factor 旋转因子构造
    */
...
void fft_cooley_tukey(complex_t<T>* seq, size_t length)
{
    ...
    */
    ...
    // pre-compute omega, and index to it later
...
void test_convolve_fft_2d() {
    ...
    auto test_convolve2d_func = [](
        const std::vector<double>& data, size_t data_w, size_t data_h,
        const std::vector<double>& filter, size_t filter_w, size_t filter_h,
        const bool is_correlate)
    {
        std::vector<double> output;
        std::vector<double> output_2;
...
#if 0
    {
        std::vector<complex_t<double>> v;
        for (auto& it : filter)
            v.emplace_back(it, (double)0);
...
void test_convolve_fft_3d() {
    ...
    auto test_convolve3d_func = [](
        const std::vector<double>& data, size_t data_w, size_t data_h, size_t channel,
        const std::vector<double>& filter, size_t filter_w, size_t filter_h,
        const bool is_correlate)
    {
        std::vector<double> output;
